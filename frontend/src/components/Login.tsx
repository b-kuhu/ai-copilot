...
const Login = () => {
  ...
  const submitHandler= async(e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const { data } = await axios.post(`${process.env.NEXT_PUBLIC_API_BASE_URL}/api/v1/login`, {
        email,
        password
      });
      ...
      toast.error(error.response?.data?.message[0]?.message || "An error occurred during login.");
    } finally {
      setLoading(false);
    }
  }
